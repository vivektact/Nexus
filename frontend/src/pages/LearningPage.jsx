...
const LearningPage = () => {
    ...
    // --- CORE LOGIC: API FETCHING (Vocabulary) ---
    useEffect(() => {
        const fetchAndProcessWords = async () => {
            setIsLoading(true);
            setError(null);
            try {
                // Step 1: Fetch a list of related words from Datamuse API for a dynamic pool
                const wordListResponse = await fetch('https://api.datamuse.com/words?ml=knowledge&max=30');
                if (!wordListResponse.ok) throw new Error('Failed to fetch the word list.');
                const words = await wordListResponse.json();
                const wordStrings = words.map(w => w.word);

                // Step 2: Fetch definitions for each word from the Dictionary API
                const definitionPromises = wordStrings.map(word =>
                    fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${word}`)
                        .then(res => res.json())
                        .catch(() => null) // Ignore individual fetch errors to not fail the whole process
                );

                const results = await Promise.all(definitionPromises);

                // Step 3: Format and filter the results into a clean data structure
                const formattedWords = results
                    .filter(result => result && !result.title) // Filter out errors and "No Definitions Found"
                    .map(result => {
                        const wordData = result[0];
                        const meaning = wordData.meanings[0];
                        const definition = meaning?.definitions[0];

                        if (!definition) return null; // Skip if the entry has no valid definition
                ...
                //     throw new Error("Not enough valid words could be fetched for the page.");
                // }

                // This is the full pool of words for dynamic quizzes
                setFullWordList(formattedWords); 
                // Set the "daily" words for display from the first 6 fetched words
    ...
    // --- CORE LOGIC: Set specific YouTube Video ---
    useEffect(() => {
        // Hardcode the specific YouTube video
        setYoutubeVideos([
            {
    ...
    const generateQuiz = () => {
        if (fullWordList.length < 5) return;

        // Get 5 random words from the full pool for the quiz questions
        const wordsForQuiz = [...fullWordList].sort(() => 0.5 - Math.random()).slice(0, 5);
        
        const newQuizData = wordsForQuiz.map(correctWord => {
            let options = [correctWord];
            // Get 3 other random words to use for incorrect options
            while (options.length < 4) {
                const randomIndex = Math.floor(Math.random() * fullWordList.length);
                const randomWord = fullWordList[randomIndex];
                // Ensure the random word isn't already in the options
            ...
            return {
                ...
                // Use the definitions from the 4 selected words and shuffle them
                ...
                return (
                    <div>
                        ...
                        <div className="mt-6 flex justify-center">
                            <button onClick={handleSubmitQuiz} className="bg-indigo-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-indigo-700 transition-colors">Submit Answers</button>
                        </div>
                    </div>
                );
            case 'intro':
            default:
                return (
                    <div className="text-center py-10">
                        ...
                        <button onClick={handleStartNewQuiz} disabled={isLoading || error} className="bg-indigo-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-indigo-700 transition-colors disabled:bg-slate-500 disabled:cursor-not-allowed">
    ...
    const styles = `
        ...
        .correct-answer { background-color: #16a3a4 !important; color: white !important; border-color: #16a3a4 !important; }
        .incorrect-answer { background-color: #dc2626 !important; color: white !important; border-color: #dc2626 !important; }
        .video-container {
            position: relative;
            width: 100%;
